'''
    High level module for interacting with DrRobot X80Pro robot.
...
from numpy import array, pi, poly1d, polyval

from DrRobot import Platform


class X80Pro(Platform):
    ''' A high level DrRobot x80Pro interface.'''
    
    # robot's structural parameters
    wheelsDistance = 1.6 * 0.295      # distance between the wheels [m]
    wheelDiameter = 2 * 0.0825  # wheel radius [m]
    pulsesPerRevolution = 756   # number of pulses per wheel revolution
#     pulse = 2657.218777679362 #2264.1509433962265 # 1 m [pulse] = N/(pi*wheelDiameter)
#     pulse = 1 #pulsesPerRevolution / (pi*wheelDiameter) # 1 m [pulse] = N/(pi*wheelDiameter)
#     pulse = pulsesPerRevolution / (pi*wheelDiameter) # 1 m [pulse] = N/(pi*wheelDiameter)
    pulse  = 3200
    NoIR = 7
    NoUS = 6
    maxSpeed = 3200 # maximum speed [pulses]
    minSpeed = 400  # minimum speed [pulses]
    
    vel2pulse_model = poly1d([2473.47492501, 282.17373324])
...
#         return int(vel*cls.pulse)
        return int(max(min(vel*cls.pulse, cls.maxSpeed), -cls.maxSpeed))
#         return int(max(min(polyval(cls.vel2pulse_model, vel), cls.maxSpeed),
#                        -cls.maxSpeed))
    ...
    def getIR(self):
        ...
        with self._lock:
            return array(map(X80Pro.ad2ir, [self.ir] + self.customAD[2:]))
    ...
    def setVelocity(self, linear, angular):
        ...
        self.setSpeed([linear - (X80Pro.wheelsDistance*angular)/2,  # left
                       linear + (X80Pro.wheelsDistance*angular)/2]) # right
        print 'Final Angle Control: ' , (X80Pro.wheelsDistance*angular)
    
    def setSpeed(self, speed, timePeriod=None):
        '''Sets the speed of the two motors.'''
        if len(speed) != 2:
            raise ValueError('invalid number of values in list')
        sLeft, sRight = X80Pro.vel2pulse(speed[0]), X80Pro.vel2pulse(speed[1])
        print 'speed: ', speed, '[m/s]', (sLeft, sRight), '[pulse/s]'
        if timePeriod:
